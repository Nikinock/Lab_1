...
var modal = new _bootstrap.Modal(_jquery(".modal"));
var modal_card_now = _jquery(".card");
function fillModal(source) {
    modal.hide();
    _jquery(document).find(_jquery(".modal-header")).html(source.find(".card-title").text());
    _jquery(document).find(_jquery(".modal-body")).html(source.find(".card-text").text());
    modal_card_now = source;
}
function modal_next() {
    if (modal_card_now.parent().next().children().length == 0) fillModal(modal_card_now.parent().parent().children().first().children());
    else fillModal(modal_card_now.parent().next().children());
    modal = new _bootstrap.Modal(_jquery(".modal"));
    modal.show();
}
function modal_prev() {
    if (modal_card_now.parent().prev().children().length == 0) fillModal(modal_card_now.parent().parent().children().last().children());
    else fillModal(modal_card_now.parent().prev().children());
    modal = new _bootstrap.Modal(_jquery(".modal"));
    modal.show();
}
_jquery("#load-toast").on("click", function(e) {
    var toast = new _bootstrap.Toast(_jquery(".toast"));
    toast.show();
});
_jquery(".card").on("click", function(e) {
    fillModal(_jquery(this));
    console.log(_jquery(this));
    modal = new _bootstrap.Modal(_jquery(".modal"));
    modal.show();
});
_jquery(".btn-next").on("click", function(e) {
    modal_next();
});
_jquery(".btn-pre").on("click", function(e) {
    modal_prev();
});
_jquery(document).keydown(function(e) {
    if (e.key === "ArrowLeft") modal_next();
    else if (e.key === "ArrowRight") modal_prev();
});

},{"jquery":"hgMhh","bootstrap":"h36JB"}],"hgMhh":[function(require,module,exports) {
/*!
...
},{"./createPopper.js":"cHuNp","./modifiers/eventListeners.js":"hBKsL","./modifiers/popperOffsets.js":"6I679","./modifiers/computeStyles.js":"gDlm2","./modifiers/applyStyles.js":"4iMn4","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}]},["cu6sN","bAabt"], "bAabt", "parcelRequire880c")
